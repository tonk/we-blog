...
B<we-blog-add> adds a blog post or a page to the We-Blog repository. If
...
Tells B<we-blog-add> to add a page or pages.

=item B<-P>, B<--post>, B<--posts>

Tells B<we-blog-add> to add a blog post or blog posts. This is the default
option.
...
B<we-blog-init>(1), B<we-blog-config>(1), B<we-blog-edit>(1), B<we-blog-remove>(1),
B<we-blog-make>(1)
